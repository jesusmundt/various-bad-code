...
rgbvalues getBackground(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int k = 0; k < (height); k++)
    {
        for (int l = 0; l < (width); l++)
        {
            ...
            if ((image[k][l].rgbtBlue + image[k][l].rgbtGreen + image[k][l].rgbtRed) / 3 < 20 || (image[k][l].rgbtBlue + image[k][l].rgbtGreen + image[k][l].rgbtRed) / 3 > 220 )
            {
                for (int m = k; m < k + 10; m++)
                {
                    ...
                    average.blue = sum.blue / 100;
                    average.green = sum.green / 100;
                    average.red = sum.red / 100;
                ...
                if (abs(average.blue - image[k][l].rgbtBlue) < 30 && abs(average.green - image[k][l].rgbtGreen) < 30 && abs(average.red - image[k][l].rgbtRed) < 30)
                {
