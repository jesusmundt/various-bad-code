...
rgbvalues getBackground(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int k = 0; k < (height); k++)
    {
        for (int l = 0; l < (width); l++)
        {
            ...
            if ((image[k][l].rgbtBlue + image[k][l].rgbtGreen + image[k][l].rgbtRed) / 3 < 100 || (image[k][l].rgbtBlue + image[k][l].rgbtGreen + image[k][l].rgbtRed) / 3 > 200 )
            {
                for (int m = k - 5; m < k + 10; m++)
                {
                    for (int n = l; n < l + 15; n++)
                    {
                ...
                if (abs(average.blue - image[k][l].rgbtBlue) < 50 && abs(average.green - image[k][l].rgbtGreen) < 50 && abs(average.red - image[k][l].rgbtRed) < 50 )
                {
