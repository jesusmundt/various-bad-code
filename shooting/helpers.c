...
rgbvalues getBackground(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < (height/3); i++)
    {
        ...
        averageB.blue = sumB.blue / (height*width/9);
        averageB.green = sumB.green / (height*width/9);
        averageB.red = sumB.red / (height*width/9);
    ...
    for (int k = 0; k < (height); k++)
    {
        for (int l = 0; l < (width); l++)
        {
            ...
            if ((image[k][l].rgbtBlue + image[k][l].rgbtGreen + image[k][l].rgbtRed) / 3 < (averageB.blue + averageB.green + averageB.red) / 3 || (image[k][l].rgbtBlue + image[k][l].rgbtGreen
                + image[k][l].rgbtRed) / 3 > 220 )
            {
